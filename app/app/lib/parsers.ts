...
export function parseText(text: string, title?: string): ParsedContent {
  ...
  for (const line of lines) {
    ...
    if (headingMatch) {
      if (currentParagraph.trim()) { paragraphs.push(currentParagraph.trim()); currentParagraph = ''; }
      headings.push({ level: headingMatch[1].length, text: headingMatch[2] });
      continue;
    }
    if (/^[=]{3,}$/.test(trimmed) && paragraphs.length > 0) { const lastPara = paragraphs.pop()!; headings.push({ level: 1, text: lastPara }); continue; }
    if (/^[-]{3,}$/.test(trimmed) && paragraphs.length > 0) { const lastPara = paragraphs.pop()!; headings.push({ level: 2, text: lastPara }); continue; }
  ...
  return { title: title || headings[0]?.text || 'Untitled', text, headings, paragraphs };
}
...
export function parseHTML(html: string): ParsedContent {
  ...
  const canonicalMatch = html.match(/<link[^>]*rel=["']canonical["'][^>]*href=["']([^"']+)["']/is) || html.match(/<link[^>]*href=["']([^"']+)["'][^>]*rel=["']canonical["']/is);
  ...
  while ((match = jsonLdRegex.exec(html)) !== null) {
    try {
      ...
      if (Array.isArray(data['@graph'])) data['@graph'].forEach((item: { '@type'?: string }) => { if (item['@type']) schemaTypes.push(item['@type']); });
  ...
  return {
    title, text: fullText, headings, paragraphs, html,
    meta: {
