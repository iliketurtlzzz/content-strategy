...
const seoChecklist = [
  ...
  "Internal linking: Minimum 2-3 links to other pages per blog post",
...
const formattingStandards = [
  ...
  { label: "Links", detail: "Internal links to services/blog; external to authoritative sources" },
];
...
export default function BrandGuidelinesPage() {
  ...
  return (
    <div className="p-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className={`text-2xl font-bold ${dark ? "text-white" : "text-slate-900"}`}>{activeClient.name} Brand Guidelines</h1>
        <p className={`mt-1 text-sm ${dark ? "text-slate-400" : "text-slate-500"}`}>{activeClient.name} brand voice, editorial standards, and content requirements</p>
      </div>
      ...
      {activeTab === "voice" && (
        <div className="space-y-6">
          ...
          <div className={`rounded-xl border p-6 shadow-sm ${dark ? "bg-[#111827] border-[#1e293b]" : "bg-white border-slate-200"}`}>
            <h2 className={`text-lg font-bold mb-1 ${dark ? "text-white" : "text-slate-900"}`}>{activeClient.name} Brand Voice</h2>
            <p className={`text-sm mb-6 ${dark ? "text-slate-400" : "text-slate-500"}`}>
              {activeClient.industry} — {activeClient.website}
            </p>
            <p className={`text-sm rounded-lg p-4 border ${
              ...
              <span className={`font-semibold ${dark ? "text-blue-400" : "text-blue-700"}`}>Positioning:</span> {activeClient.brandPositioning}
            </p>
          </div>
          ...
          <div className={`rounded-xl border shadow-sm ${dark ? "bg-[#111827] border-[#1e293b]" : "bg-white border-slate-200"}`}>
            ...
            <div className={`divide-y ${dark ? "divide-[#1e293b]" : "divide-slate-100"}`}>
              {activeClient.toneAttributes.map((attr) => (
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className={`rounded-xl border shadow-sm ${dark ? "border-emerald-500/20 bg-[#111827]" : "border-emerald-200 bg-white"}`}>
              ...
              <div className={`divide-y p-2 ${dark ? "divide-[#1e293b]" : "divide-slate-50"}`}>
                {activeClient.onVoiceExamples.map((ex, i) => (
            ...
            <div className={`rounded-xl border shadow-sm ${dark ? "border-red-500/20 bg-[#111827]" : "border-red-200 bg-white"}`}>
              ...
              <div className={`divide-y p-2 ${dark ? "divide-[#1e293b]" : "divide-slate-50"}`}>
                {activeClient.offVoiceExamples.map((ex, i) => (
                  <div key={i} className="flex items-start gap-3 px-4 py-3">
        ...
        <div className="space-y-6">
          <div className={`rounded-xl border shadow-sm ${dark ? "bg-[#111827] border-[#1e293b]" : "bg-white border-slate-200"}`}>
            ...
            <div className="grid grid-cols-1 gap-4 p-6 sm:grid-cols-2 lg:grid-cols-3">
              {activeClient.services.map((svc, i) => (
          ...
          <div className={`rounded-xl border shadow-sm ${dark ? "bg-[#111827] border-[#1e293b]" : "bg-white border-slate-200"}`}>
            ...
            <div className="p-6">
              <div className="space-y-3">
                {activeClient.differentiators.map((diff, i) => (
            ...
            <div className="p-6">
              <div className="space-y-1">
                {[
                  ...
                  { label: "CTA", detail: "What to do next (contact, download resource, read related post)" },
                ].map((item, i) => (
